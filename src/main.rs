...
//TO-DO:
//Add velocity to paddles, add a score counter, and add some sound.
...
impl GameState {
    fn new(ctx: &mut Context) -> tetra::Result<GameState> {
        let player1_texture = Texture::new(ctx, "./res/player1.png")?;
        let player1_position = Vec2::new(
            16.0,
            (WINDOW_HEIGHT - player1_texture.height() as f32) / 2.0,
        );

        let player2_texture = Texture::new(ctx, "./res/player2.png")?;
...
impl State for GameState {
    fn update(&mut self, ctx: &mut Context) -> tetra::Result {
        ...
        if let Some(paddle) = paddle_hit {
            ...
            //Find the offset between the paddle and ball ranging between -1.0 and 1.0.
